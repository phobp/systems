...
{
    bool header, segments, membrief, memfull, disas_code, disas_data = false;
    elf_hdr_t hdr;
    memory_t memory;
    char *filename = NULL;

    // Parse command line and check for failure
    if (!parse_command_line_p3(argc, argv, &header, &segments, &membrief, &memfull, &disas_code, &disas_data, &filename)) {
        exit(EXIT_FAILURE);
    }
